...
class COSBackend:
    ...
    def __init__(self, cos_config):
        service_endpoint = cos_config.get(
            'endpoint').replace('http:', 'https:')
        service_key = cos_config.get('secret_key')
        acces_key = cos_config.get('acces_key')
    ...
    def put_object(self, bucket_name, key, data):
        ...
        try:
            res = self.cos_client.put_object(
                Bucket=bucket_name, Key=key, Body=data)
            status = 'OK' if res['ResponseMetadata']['HTTPStatusCode'] == 200 else 'Error'
            try:
                print('PUT Object {} - Size: {} - {}'.format(key,
                                                             sizeof_fmt(len(data)), status))
            except:
    ...
    def list_objects(self, bucket_name, prefix=None):
        paginator = self.cos_client.get_paginator('list_objcets_v2')
        try:
